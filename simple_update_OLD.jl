...
function simple_update(psi, H, dτ, χ, max_iterations, ctm_alg, Js; χenv = 3*χ, translate = false)
    ...
    (psi, lambdas) = gauge_fix(psi, lambdas) # check whether correct
    for i = 1:max_iterations
        # println("Started with iteration i = $(i)")
        psi_old = copy(psi)
        for i = 1:4
            check_norm(psi)
            (psi, lambdas) = gauge_fix(psi, lambdas) # twice?
            check_norm(psi)
...
(psi, lambdas) = simple_update(psi, H, dτ, χ, max_iterations, ctm_alg, Js; translate = true);
psi = do_CTMRG(psi, H, ctm_alg, χenv)
(psi, lambdas) = simple_update(psi, H, dτ, χ, max_iterations, ctm_alg, Js; translate = true);
